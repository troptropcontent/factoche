module Organization
  module ProjectVersions
    class CreateCompletionSnapshotInvoice
      class << self
        def call(project_version_id, invoice_amounts)
          ...
          ensure_original_item_uuid_belongs_to_project_version!(project_version, invoice_amounts)

          ensure_invoiced_item_remains_within_limits!(invoice_amounts, project_version.items, project.id, company.id)

          accounting_service_arguments = build_accounting_service_arguments(company, client, project_version, invoice_amounts)
        ...
        def ensure_original_item_uuid_belongs_to_project_version!(project_version, invoice_amounts)
          unless invoice_amounts.all? { |invoice_amount| project_version.items.where({ original_item_uuid: invoice_amount[:original_item_uuid] }).exists? }
        ...
        def ensure_invoiced_item_remains_within_limits!(invoice_amounts, items, project_id, company_id)
          ...
          wrong_invoice_amount = invoice_amounts.find { |invoice_amount|
            ...
            previously_invoiced_amount + invoice_amount[:invoice_amount] > item_amount
          }
        ...
        def create_invoice!(args)
          result = Accounting::FinancialTransactions::CreateCompletionSnapshotInvoice.call(*args)
