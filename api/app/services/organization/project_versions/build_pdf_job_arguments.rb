    ...
    class BuildPdfJobArguments
      class << self
        ...
        def find_print_url(version)
          Rails.application.routes.url_helpers.quote_prints_url(version.project.id, version.id, { host: ENV.fetch("FABATI_PRINT_MICROSERVICE_HOST"), port: ENV.fetch("FABATI_PRINT_MICROSERVICE_PORT") })
        end
      end
    end
  end
