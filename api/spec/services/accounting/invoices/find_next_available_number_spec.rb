...
module Accounting
  module Invoices
    RSpec.describe FindNextAvailableNumber do
      describe ".call" do
        let(:company_id) { 1 }
        let(:issue_date) { Time.new(2024, 3, 20) }

        context "when successful" do
          context "when published argument is true" do
            it "generates the first invoice number when no unpublished invoices exist", :aggregate_failures do
              result = described_class.call(company_id: company_id, published: true, issue_date: issue_date)
            ...
            it "generates the next invoice number based on existing unpublished invoices", :aggregate_failures do
              ...
              expect(result).to be_success
              expect(result.data).to eq("INV-2024-000003")
            end
            ...
            it "only counts invoices for the specified company", :aggregate_failures do
              ...
              FactoryBot.create(:invoice, :posted, company_id: 2, holder_id: 1, number: "INV-2024-00001", issue_date: issue_date - 2.days)
              FactoryBot.create(:invoice, :cancelled, company_id: 2, holder_id: 1, number: "INV-2024-00002", issue_date: issue_date - 2.days)
            ...
            it "uses the provided issue date for the year", :aggregate_failures do
              ...
              result = described_class.call(company_id: company_id, published: true, issue_date: future_date)

              expect(result).to be_success
              expect(result.data).to eq("INV-2025-000001")
            end
          ...
          context "when published argument is false" do
            it "generates the first invoice number when no unpublished invoices exist", :aggregate_failures do
              result = described_class.call(company_id: company_id, published: false, issue_date: issue_date)
            ...
            it "uses the provided issue date for the year", :aggregate_failures do
              future_date = Time.new(2025, 1, 1)
              result = described_class.call(company_id: company_id, published: false, issue_date: future_date)
        ...
        context "when an error occurs" do
          it "returns a failure result", :aggregate_failures do
            allow(Invoice).to receive(:where).and_raise(StandardError.new("Database error"))
