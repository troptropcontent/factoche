...
RSpec.describe Organization::Quotes::Create do
  describe '.call' do
    ...
    context 'when params are valid' do
      ...
      it 'creates a quote with all associated records' do
        expect { result }.to change(Organization::Quote, :count).by(1)
          ...
          .and change(Organization::ItemGroup, :count).by(2)
    ...
    context 'with transaction rollback' do
      before do
        allow(Organization::Item).to receive(:create!).and_raise(ActiveRecord::RecordInvalid)
      end
