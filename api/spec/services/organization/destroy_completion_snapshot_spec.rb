...
RSpec.describe Organization::DestroyCompletionSnapshot do
  ...
  describe "destroying completion snapshot" do
    ...
    context "when the completion snapshot is not draft" do
      let!(:completion_snapshot) { FactoryBot.create(:completion_snapshot, project_version: project_version, invoice: FactoryBot.create(:invoice)) }
      ...
      it "raises an error and keeps the record in the database", :aggregate_failures do
        expect {
          expect {
            ...
            "Cannot delete completion snapshot with status 'invoiced'. Only snapshots in 'draft' status can be deleted"
          )
