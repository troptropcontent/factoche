...
RSpec.describe Organization::ProjectVersions::CreateCompletionSnapshotInvoice do
  include_context 'a company with a project with three item groups'
  describe '.call' do
    subject(:result) { described_class.call(project_version.id, invoice_amounts) }

    let(:invoice_amounts) do
      [ {
    ...
    context 'when validations fail' do
      ...
      context 'when invoice amount exceeds item total amount' do
        let(:previous_invoice_amounts) do
        end

        before do
          # Create a previous invoice for this item
          described_class.call(project_version.id, [ {
            original_item_uuid: first_item.original_item_uuid,
