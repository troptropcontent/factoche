...
RSpec.shared_context 'a company with some quotes' do |number_of_quotes: 1|
  ...
  number_of_quotes.times do |number_of_quote_index|
    ...
    let("#{ordinals[number_of_quote_index]}_quote") { Organization::Quotes::Create.call(company.id, send("#{ordinals[number_of_quote_index]}_client").id, create_quote_params).data }
